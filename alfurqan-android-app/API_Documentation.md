...
## 06/09/2026 (v1.6.14+43) — 3 DEEP-fix: duplicate/again order-number, Track Order specific order nahi kholta tha, ordered-but-history-me-missing + ADDRESS REMOVE ko aur 9 naye attack shapes

**BUG-A (screenshot proof — dono Order Placed pages par SAME "#1041"; asal orders #1041 + #1042 the):**
ROOT: purana success-number fallback `GetUserOrders` ke SAB rows me se MAX number uthata tha. Server naye order ka `order_number` TURANT assign nahi karta (naya row pehle number ke BINA aati hai, number baad me lagta hai) — isliye max() hamesha PICHLE numbered order (#1041) hi pakad leta. Doosra order bhi "#1041" dikhane laga = "many have same id".
**FIX (snapshot-identity):** order place karne se PEHLE existing orders ke **PK (`id`) snapshot** lete hai; success ke baad GetUserOrders ~9sec tak POLL karke wo NAYI row dhundte hai jo snapshot me nahi thi (PK se — kabhi galat-order pakad hi nahi sakta) → usi row ka `order_number` aate hi display. Number na mile to usi NAYI row ka PK dikhate hai (history card ka fallback bhi wahi PK hota hai — dono jagah SAME, sachcha, unique). Response-parse bhi order_number PEHLE / PK baad me + `isOrderNumber` flag.

**BUG-B ("success page ke Track Order → specific order nahi, list khulti thi"):** Track Order ab placed order ka REAL `order_number` ho to history ke UPAR usi order ki **ORDER DETAIL** kholta hai (postFrame push — offAll race nahi); back → list (natural flow). Number assign na hua ho to purana behavior (sirf list).

**BUG-C ("order place hua par history me nahi dikha"):** fetchOrders ab `page=1&paginate=100` bhejta hai (default page-size me rows cut ho sakti thi) + server se duplicate aati rows (pagination overlap) display-identity se **dedupe** (one order = one card, "same id" cards khatam) + history model ka `orderId` ab **order_number-first** — card title (jo `#1042` dikhata hai) aur detail call (GetOrder `?id=order_number` hi asli detail deta hai) ab FIRST try me match.
