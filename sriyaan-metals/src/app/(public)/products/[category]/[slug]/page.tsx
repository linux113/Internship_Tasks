...
export default async function ProductDetail({
  ...
  const specs = [
    ...
    ["SKU", product.sku],
    ...product.specifications.map((item: { label: string; value: string }) => [item.label, item.value]),
        ...
        <div>
          ...
          {product.images.length > 1 && (
            <div className="mt-3 grid grid-cols-4 gap-2">
              {product.images.map((img: { id: string; url: string; alt: string }) => (
        ...
        {product.documents.length > 0 && (
          <div className="mt-10">
            ...
            <ul className="mt-3 space-y-2">
              {product.documents.map((d: { id: string; url: string; name: string }) => (
                <li key={d.id}>
