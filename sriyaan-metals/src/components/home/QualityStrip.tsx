    ...
    <section className="surface-paper py-24">
      <div className="container-site">
        ...
        <div className="mt-14 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {items.length === 0 && (
            <div className="col-span-2 md:col-span-3 lg:col-span-6 bg-white border border-black/8 p-8">
              <p className="font-display text-xl uppercase tracking-wide text-ink">No certificates published yet</p>
              <p className="mt-2 text-sm text-[#6b7280]">Certificates appear only after an administrator uploads verified information.</p>
            </div>
          )}
          {items.map((item, i) => (
