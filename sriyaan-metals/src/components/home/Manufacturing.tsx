    ...
    <ParallaxRoot className="relative min-h-[110vh] overflow-hidden bg-ink">
      ...
      >
        <div className="aspect-[4/5] overflow-hidden border border-white/15">
          <img src={fg} alt="Industrial metalwork — representative image" className="h-full w-full object-cover" />
        </div>
      </ParallaxLayer>
      ...
      <div className="relative z-10 container-site py-28">
        ...
        <h2 className="display text-5xl md:text-7xl mt-4 max-w-3xl">
          Infrastructure, documented clearly
        </h2>
        <p className="mt-6 max-w-xl text-haze text-lg">
          Facility, machinery, warehouse, packaging and inspection information
          can be published here from the CMS after it is supplied and verified.
        </p>
        <div className="mt-14 grid sm:grid-cols-2 lg:grid-cols-3 gap-4 max-w-4xl">
          {images.length > 0 ? images.slice(0, 6).map((img) => (
            ...
            <article className="sm:col-span-2 border border-white/10 bg-ink/55 backdrop-blur-sm p-6">
              <p className="font-display tracking-wider uppercase text-brass">Awaiting verified content</p>
              <p className="mt-2 text-sm text-mist">No facility or capability claim is published until the client adds it in Admin → Content.</p>
            </article>
          )}
        </div>
